  ...
  <div class="flex flex-col h-screen bg-background text-foreground overflow-hidden">
    <header class="h-16 border-b flex items-center justify-between px-6 bg-card shrink-0">
      ...
      <div class="flex items-center gap-4">
        ...
        <ModeToggle />
        <Button variant="ghost" size="icon" @click="$router.push('/home')" title="Sair do PDV">
          <LogOut class="size-5" />
        </Button>
      </div>
    </header>
    ...
    <main class="flex-1 flex overflow-hidden p-4 gap-4 bg-muted/10">
      <div class="flex-3 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div v-else-if="filteredProducts.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          <Card
            ...
            :key="product.uuid"
            class="group cursor-pointer hover:ring-2 hover:ring-primary transition-all shadow-sm flex flex-col overflow-hidden"
            @click="addToCart(product)"
          >
            <div class="aspect-square bg-muted relative flex items-center justify-center border-b">
              <img v-if="product.image" :src="product.image" class="object-cover size-full group-hover:scale-110 transition-transform duration-300" />
            ...
            <CardContent class="p-3 flex-1 flex flex-col justify-between bg-card">
              <div>
                <p class="text-[10px] font-bold text-primary uppercase tracking-wider mb-1">{{ product.category?.name || 'Geral' }}</p>
                <h3 class="font-bold text-sm leading-tight line-clamp-2 uppercase">{{ product.name }}</h3>
              </div>
        ...
        <div v-else class="h-full flex flex-col items-center justify-center text-muted-foreground opacity-50">
      ...
      <aside class="flex-1 bg-card rounded-xl border-2 flex flex-col shadow-2xl overflow-hidden max-w-100">
        ...
        <div class="flex-1 overflow-y-auto px-4 custom-scrollbar">
          <div v-for="(item, index) in cart" :key="item.uuid" class="py-4 border-b last:border-0 flex gap-3 group">
            <div class="size-12 rounded bg-muted border shrink-0 flex items-center justify-center text-xs font-bold">
              {{ item.cartQuantity }}x
            </div>
            ...
            <div class="text-right flex flex-col justify-between items-end">
              <span class="font-bold text-sm">{{ formatPrice(item.price * item.cartQuantity) }}</span>
              <button @click="removeFromCart(index)" class="text-destructive hover:underline text-[10px] font-bold uppercase">Excluir</button>
            </div>
          </div>
...
import {
  Search, Plus, ShoppingCart, Package,
  LogOut, RefreshCw, SearchX
} from 'lucide-vue-next'
import ModeToggle from '@/components/ModeToggle.vue'
import api from '@/services/api'

// Importando os tipos do seu projeto
import type { Product } from '@/components/products/columns'

// Ajustando a interface Category para usar uuid conforme o padrão do Product
...
const fetchPDVData = async () => {
  ...
  try {
    ...
    // Mapeando categorias para garantir que temos o uuid correto
...
const filteredProducts = computed(() => {
  return products.value.filter(p => {
    ...
    // Removido 'sku' pois não existe na interface Product fornecida
    const matchesSearch = p.name.toLowerCase().includes(searchLower)
    const matchesCategory = selectedCategory.value === 'all' || p.category?.uuid === selectedCategory.value
    return matchesSearch && matchesCategory
  })
})

const cartTotal = computed(() => cart.value.reduce((acc, item) => acc + (item.price * item.cartQuantity), 0))
...
const addToCart = (product: Product) => {
  ...
  if (existing) {
    ...
    // Adicionamos a propriedade cartQuantity para controle interno do carrinho
